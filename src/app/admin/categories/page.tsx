...
import { Plus, Pencil, Trash2, FolderTree, Upload, Tag as TagIcon, Star } from "lucide-react";
...
export default function AdminCategoriesPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Section switcher */}
      <div className="inline-flex items-center gap-1 rounded-2xl border border-cream-200 bg-cream-50 p-1">
        {tabs.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={
                active
                  ? "inline-flex items-center gap-2 rounded-xl bg-saffron-500 px-4 py-2 text-sm font-semibold text-white shadow-soft"
                  : "inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold text-ink-500 hover:text-maroon-900"
              }
            >
              <Icon size={16} /> {label}
            </button>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-500">
            ...
            a product can belong to several.
          </p>
        </div>
